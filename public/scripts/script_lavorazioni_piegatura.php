<script>
	...
	function SendData() {
		...
		$.ajax({
			url: 'ajax/ajax_lavorazioni_piegatura.php',
			type: 'POST',
			data: {
				aggiunta_riga : "True",
			...
			success: function(response) {
				...
				var table = $('#datatable-lavorazioni_piegatura').DataTable();
				...
				var newRow = [
					...
					formValues['segnatura'], // Inizio segnatura
					formValues['copie_inizio'], // Fogli Da Lavorare
					formValues['copie_fine'], // Fogli Lavorati
					macchina_condivisa, // Completato
					segnatura_finita, // Completato
	...
	function modificaCampoNormale(fieldName, value, id) {
		$.ajax({
			...
			success: function (response) {
				...
				//===================== casistiche speciali ==============================
				if(fieldName === "macchina_condivisa" || fieldName === "segnatura_finita"){
					value = (value === "1") ? "Sì" : "No";
				}
