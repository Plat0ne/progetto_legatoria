    ...
    <div class="container-fluid mt-1 h-100">
        ...
        <div id="lavorazioniContainer">
            @foreach($lavorazioni as $l)
                <div id="carta_lavorazione_{{ strtolower($l['id_lc']) }}" class="card lavorazione-item">
                    <div class="card-body">
                        <div class="row mb-2 justify-content-center">
                            ...
                            <div class="row">
                                ...
                                <div class="col">
                                    Macchina Condivisa: <br>
                                    <h2 class="fw-bold">{{ $l['macchina_condivisa'] == 1 ? 'Sì' : 'No' }}</h2>
                                </div>
                            </div>
                        </div>
                        <div class="row mb-2 justify-content-between">
                            <div class="col">
                                <button type="button" class="btn btn-warning w-100 text-dark" onclick="init_modal_fine('{{ $l['id_lc'] }}', '{{ $l['codice_operatore'] }}')"><i class="fas fa-sign-out-alt"></i>&nbsp;ESCI</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        @include('produzione.modals.modal_uscita_cucitura')
        @include('produzione.modals.modal_entrata_cucitura')
...
<script>
    ...
    function init_modal_fine(id_lavorazione, codice_operatore) {
        ...
        document.getElementById('colpi_fine').value = '';
    ...
    $(document).ready(function() {
        ...
        $('#bottone_esci').on('click', function(e) {
            e.preventDefault();
            var colpi_fine = $('#colpi_fine').val();
            var id_lavorazione = $('#id_lavorazione').val();
            var error_msg = $('#error_msg_uscita');

            error_msg.html('');
            $('#colpi_fine').removeClass('text-danger');

            // if (isNaN(colpi_fine) || colpi_fine <= 0) {
            //     $('#colpi_fine').addClass('text-danger');
            //     error_msg.html('Inserire un valore numerico maggiore di zero');
            //     return;
            // }
            $.ajax({
                type: 'POST',
                url: '/produzione/cucitura/uscita/' + id_lavorazione,
                data: {
                    '_token': '{{ csrf_token() }}',
                    'colpi_fine': colpi_fine,
    ...
    //scrpit per la parte entrata __________________________________________
        $('#bottone_inizia').on('click', function(event) {
            ...
            var colpi_inizio = $('#colpi_inizio').val();
            var macchina_condivisa = $('#macchina_condivisa').is(':checked') ? 1 : 0;
            ...
            if (isNaN(colpi_inizio) || colpi_inizio < 0) {
                messaggi_errore.push('Inserire un numero di colpi maggiore di zero');
            }
