    ...
    <div class="container-fluid mt-1 h-100">
        ...
        <div id="lavorazioniContainer">
            @foreach($lavorazioni as $l)
                <div id="carta_lavorazione_{{ strtolower($l['id_lr']) }}" class="card lavorazione-item">
                    <div class="card-body">
                        ...
                        <div class="row mb-2 justify-content-between">
                            <div class="col">
                                <button type="button" class="btn btn-warning w-100 text-dark" onclick="init_modal_fine('{{ $l['id_lr'] }}', '{{ $l['codice_operatore'] }}')"><i class="fas fa-sign-out-alt"></i>&nbsp;ESCI</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        @include('produzione.modals.modal_entrata_raccolta')
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#bottone_esci').on('click', function(e) {
            ...
            $.ajax({
                type: 'POST',
                url: '/produzione/raccolta/uscita/' + id_lavorazione,
                data: {
                    '_token': '{{ csrf_token() }}',
    ...
    //scrpit per la parte entrata __________________________________________
        $('#bottone_inizia').on('click', function(event) {
            ...
            $.ajax({
                type: 'POST',
                url: '/produzione/raccolta/entrata',
                data: {
                    '_token': '{{ csrf_token() }}',
