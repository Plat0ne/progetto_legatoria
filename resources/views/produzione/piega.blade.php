    ...
    <div class="container-fluid mt-1 h-100">
        ...
        <div id="lavorazioniContainer">
            @foreach($lavorazioni as $l)
                <div id="carta_lavorazione_{{ strtolower($l['id_lp']) }}" class="card lavorazione-item">
                    <div class="card-body">
                        ...
                        <div class="row mb-2 justify-content-between">
                            <div class="col">
                                <button type="button" class="btn btn-warning w-100 text-dark" onclick="init_modal_fine('{{ $l['id_lp'] }}', '{{ $l['codice_operatore'] }}')"><i class="fas fa-sign-out-alt"></i>&nbsp;ESCI</button>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        @include('produzione.modals.modal_uscita_piega')
        @include('produzione.modals.modal_entrata_piega')
...
<script>
    ...
    function init_modal_fine(id_lavorazione, codice_operatore) {
        ...
        document.getElementById('copie_lavorate_fine').value = '';
    ...
    $(document).ready(function() {
        ...
        $('#bottone_esci').on('click', function(e) {
            ...
            // if (isNaN(copie_lavorate_fine) || copie_lavorate_fine <= 0) {
            //     $('#copie_lavorate_fine').addClass('text-danger');
            //     error_msg.html('Inserire un valore numerico maggiore di zero');
            //     return;
            // }
            $.ajax({
                type: 'POST',
                url: '/produzione/piega/uscita/' + id_lavorazione,
                data: {
                    '_token': '{{ csrf_token() }}',
                    'copie_lavorate_fine': copie_lavorate_fine,
    ...
    //scrpit per la parte entrata __________________________________________
        $('#bottone_inizia').on('click', function(event) {
            ...
            var segnatura = $('#segnatura').val();
            var numero_copie_inizio = $('#numero_copie_inizio').val();
            ...
            $.ajax({
                ...
                data: {
                    ...
                    'segnatura': segnatura,
                    'n_copie_inizio': numero_copie_inizio,
